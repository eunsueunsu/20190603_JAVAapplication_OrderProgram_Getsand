...
public class SauceSelect extends ToppingSelect {
	...
	public JLabel sauceNameLb[] = new JLabel[6]; // 메인 메뉴 이름 라벨
	public JLabel saucePriceLb[] = new JLabel[6]; // 메인 메뉴 가격 라벨
	...
	Sauce[] sauceArr = new Sauce[6]; // 메인 메뉴 객체 배열
	...
	public SauceSelect() {
		...
		for (int i = 0; i < sauceArr.length; i++) {
			...
			// sauceNameLb[num].setFont(new Font("맑은 고딕", Font.BOLD, 14));
			// sauceNameLb[num].setBounds(707, y[i] , 145, 15);
			background.add(sauceNameLb[num]);

			saucePriceLb[num] = new JLabel("");
			// saucePriceLb[num].setFont(new Font("맑은 고딕", Font.BOLD, 14));
				...
				public void actionPerformed(ActionEvent e) {
					...
					if ((JButton) obj == selectButton[num]) {
						...
						if (Sauce_count < 4) {
							...
							JOptionPane.showMessageDialog(null, "소스는 3가지 이상 선택하실 수 없습니다 .");
							Sauce_count--;
						}
		...
		check_bt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (Sauce_count > 0) {
					...
					JOptionPane.showMessageDialog(null, "소스 종류를 선택해주세요");
				}
